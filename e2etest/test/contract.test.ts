...
describe('Contract', () => {
  ...
  it('assertResponse enforces only the documented response contract', () => {
    const cases: {
      ...
      { request: { method: 'post', path: '/login' }, status: 200, body: '{"token":"t"}', contentType: 'Application/JSON; charset=utf-8' },
      { request: { method: 'post', path: '/login' }, status: 200, body: '{"token":3}', error: /POST \/login response 200 body.*token.*string/ },
      { request: { method: 'post', path: '/login' }, status: 401, body: '{}', error: /response 401.*undefined status/ },
      { request: { method: 'post', path: '/login' }, status: 200, body: '{}', contentType: null, error: /Content-Type.*missing/ },
      { request: { method: 'post', path: '/login' }, status: 200, body: '{}', contentType: 'text/plain', error: /Content-Type.*text\/plain/ },
      { request: { method: 'post', path: '/login' }, status: 200, body: 'bad', error: /invalid JSON/ },
      ...
      { request: { method: 'get', path: '/users' }, status: 200, body: '[{"score":1.5}]', error: /score.*integer/ },
      { request: { method: 'get', path: '/challenges' }, status: 200, body: '[{"flag":"allowed extra field"}]' },
      { request: { method: 'get', path: '/challenges' }, status: 200, body: '[{"id":"1"}]', error: /id.*integer/ },
      { request: { method: 'get', path: '/answers/me' }, status: 200, body: '[{"answered_at":"2026-09-22T12:30:00Z","correct":false}]' },
      { request: { method: 'get', path: '/answers/me' }, status: 200, body: '[{"answered_at":"2026-02-30T12:30:00Z"}]', error: /answered_at.*date-time/ },
      { request: { method: 'get', path: '/answers/me' }, status: 200, body: '[{"correct":"false"}]', error: /correct.*boolean/ },
      { request: { method: 'delete', path: '/challenges' }, status: 403, body: 'arbitrary', contentType: 'text/plain' },
      { request: { method: 'get', path: '/answers' }, status: 404, body: '', contentType: null },
      { request: { method: 'post', path: '/logout' }, status: 200, body: 'arbitrary', contentType: null }
    ];
...
it('baseUrl requires an explicit HTTP target and preserves a base path', () => {
  ...
  for (const input of [undefined, '', 'bad', 'file:///tmp', 'http://example.test/?a=b', 'http://example.test/#fragment', 'http://user:pass@example.test']) {
...
it('requestUrl preserves the base path and serializes query values', () => {
  for (const [base, request, expected] of [
    ...
    ['http://localhost:8080/api/', { method: 'get', path: '/answers', query: { challenge_id: -1 } }, 'http://localhost:8080/api/answers?challenge_id=-1'],
    ['http://localhost:8080/api', { method: 'get', path: '/answers', query: { challenge_id: undefined } }, 'http://localhost:8080/api/answers']
  ] as const) {
