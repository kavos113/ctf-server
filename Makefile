...
DB_TEST_WRAPS=password_worker_submit pthread_mutex_init pthread_cond_init pthread_create eventfd auth_password_hash auth_password_verify crypto_pwhash_str_alg jwt_encode_str writev calloc malloc mysql_init mysql_real_connect mysql_close mysql_thread_end \
 ...
 mysql_num_rows mysql_num_fields mysql_fetch_row mysql_fetch_lengths mysql_free_result
...
CFLAGS += $(shell pkg-config --cflags libjwt libsodium jansson)
LDFLAGS = $(shell mysql_config --libs) $(shell pkg-config --libs libjwt libsodium jansson) -pthread
