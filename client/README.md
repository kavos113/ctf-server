...
問題のdescriptionはMarkdownとして表示します。見出し、強調、リスト、引用、コード、リンク、画像、表、取り消し線に対応し、通常の改行も反映します。入力・保存はMarkdown原文のままで、編集プレビューはありません。表示時にmarkedで変換し、DOMPurifyで許可したタグ・属性だけを残してから挿入します。説明をAureliaのテンプレートとしてコンパイルしません。
